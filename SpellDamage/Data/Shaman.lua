--Удар бури:
Stormstrike = MultiParser:create(SpellDamage, {1, 2}, function(data, match)
	data.damage = match[1] + match[2]
end)

--Тотем исцеляющего потока:
HealingStreamTotem = MultiParser:create(SpellTimeHeal, {3}, function(data, match)
	data.timeHeal = match[3] * 7
end)

--Тотем магмы:
MagmaTotem = MultiParser:create(SpellTimeDamage, {4}, function(data, match)
	data.timeDamage = match[3] * 30
end)

--Тотем целительного прилива:
HealingTideTotem = MultiParser:create(SpellTimeHeal, {5}, function(data, match)
